...
// ===================================
// 1. تحديد ملفات اللغات (تأكد من وجود هذا المتغير)
// ===================================
const languageFiles = {
    'ar': 'languages/ar.json',
    'en': 'languages/en.json'
    // أضف أي لغات أخرى هنا
};


// ===================================
// 2. دالة تلوين النصوص المميزة (Highlighting Function)
// ===================================

/**
 * تبحث عن النصوص المحاطة بعلامات النجمة المزدوجة (**) داخل غالبية عناصر المحتوى
 * في الصفحة، وتحولها إلى وسم <span> لتطبيق لون التمييز.
 */
function highlightTextMarkers() {
    // التعبير النمطي: يجد **نص**، ويستخدم الصيغة غير الجشعة.
    const highlightRegex = /\*\*([^\*]+)\*\*/g; 
    const highlightClass = 'highlight-primary';

    // المحددات التي تحتوي على نصوص تحتاج إلى تلوين
    const selectors = 'p, h3, h4, h5, h6, li, span[data-lang-key], div[data-lang-key]'; 

    document.querySelectorAll(selectors).forEach(element => {
        // لتجنب استهداف العناصر التي لا يجب تغييرها (مثل عناصر التنقل والأزرار)
...
// ===================================
// 3. دالة تحميل اللغة الرئيسية (مُصححة)
// ===================================

/**
 * دالة تغيير اللغة الرئيسية: تقوم بتحميل ملف الترجمة وتحديث DOM وضبط الاتجاه.
 * @param {string} langCode - كود اللغة ('ar' أو 'en').
 */
function loadLanguage(langCode) {
    const body = document.body;
    
    // 1. تحميل ملف JSON للغة
    fetch(languageFiles[langCode])
        .then(response => response.json())
        .then(translations => {
            // 2. تحديث جميع النصوص باستخدام innerHTML
            document.querySelectorAll('[data-lang-key]').forEach(element => {
                const key = element.getAttribute('data-lang-key');
                if (translations[key]) {
                    // استخدام innerHTML ضروري لحقن النص بعلامات **
                    element.innerHTML = translations[key]; 
                }
            });
            
            // ----------------------------------------------------
            // 🔑 الخطوة الحاسمة: إعادة تشغيل التلوين باستخدام setTimeout
            // ----------------------------------------------------
            setTimeout(() => {
                highlightTextMarkers(); 
            }, 0); 
